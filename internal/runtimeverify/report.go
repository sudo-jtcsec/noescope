...
func Markdown(runtime *Runtime) []byte {
	...
	fmt.Fprintf(&out, "- Safe interfaces attempted: %d\n", summary.SafeInterfacesAttempted)
	fmt.Fprintf(&out, "- Verified observations: %d\n", summary.Verified)
	fmt.Fprintf(&out, "- Redirected / authentication required: %d\n", summary.Redirected+summary.AuthRequired)
